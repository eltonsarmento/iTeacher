...
class InstituicoesDAO extends UsuariosDAO {
    ...
    public function cadastrarBySite($input) {
        ...
        $this->system->sql->insert('usuarios', array(
            ...
            'email'             => (trim($input['email']) ? trim($input['email']) : trim($input['email_secundario'])),
            'nivel'             => 5,
            'data_cadastro'     => date('Y-m-d H:i:s'),
            'cadastro_por_id'   => 0,
        ...
        $this->system->sql->insert('usuarios_dados', array(
            ...
            'email_secundario'      => (trim($input['email_secundario']) ? trim($input['email_secundario']) : trim($input['email'])),
            'razao_social'          => trim($input['razao_social']),
            'email_secundario'      => trim($input['email_secundario']),
            'tipo_pessoa'           => 2,
            'nome_responsavel'      => (trim($input['nome_responsavel']) ? trim($input['nome_responsavel']) : trim($input['nome'])),
