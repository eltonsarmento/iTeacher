...
class Perfil extends PerfilGlobal {
	...
	protected function doVisualizar() {	
		...
		if ($id)
			$this->system->view->assign('coordenador', $this->system->usuarios->getUsuario($id, true));	
		else
			$this->system->func->redirecionar();

		$this->system->admin->topo();
		$this->system->view->display('instituicao-coordenador/perfil.tpl');
	...
	protected function doEditarDadosUsuario() {
		...
		$this->system->view->assign('coordenador', $this->system->usuarios->getUsuario($id,true));
		$this->system->admin->topo();
		$this->system->view->display('instituicao-coordenador/perfil_edicao.tpl');
		$this->system->admin->rodape();
	}
	//==================================================================
