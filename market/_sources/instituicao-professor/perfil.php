...
class Perfil extends PerfilGlobal {
	...
	protected function doVisualizar() {	
		...
		$this->system->view->display('instituicao-professor/perfil.tpl');
	...
	protected function doEditarDadosUsuario() {
		...
		if ($editar_dados) {
			if ($editar) {
				...
				if ($id) $this->system->view->assign('professor', $this->system->usuarios->getUsuario($id,true));	
			}
		}
		...
		$this->system->view->assign('professor', $this->system->usuarios->getUsuario($id,true));
		$this->system->admin->topo();
		$this->system->view->display('instituicao-professor/perfil_edicao.tpl');
		$this->system->admin->rodape();
	}
	//==================================================================
